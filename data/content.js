;(function() {
  ...
  var crawlRepositories = function(url, organization) {
    $.get(url).success(function(data) {
      ...
      saveStorage('yourRepositories', yourRepositories);
  ...
  //chrome.storage.local.get('private', function(result) {
    ...
    if (typeof simpleStorage.private !== 'undefined') {
  ...
  //chrome.storage.local.get('yourRepositories', function(result) {
    if (typeof simpleStorage.yourRepositories !== 'undefined' && simpleStorage.yourRepositories.length > 0) {
  ...
  $(document).ready(function() {
    ...
    $q.parent().append('<a class="icon icon-delete" href="#"><img src="' + self.options.closeImgUrl + '"/></a>');
        ...
        templates: {
          empty: function() {
            return '<div class="aa-branding">' +
              'With <i class="octicon octicon-heart"></i> from <a href="https://www.algolia.com/?utm_source=github-awesome-autocomplete&utm_medium=link&utm_campaign=github-awesome-autocomplete_search"><img src="'+ self.options.logoUrl +'" title="Algolia" /></a>' +
              '</div>';
          }
        }
      }
